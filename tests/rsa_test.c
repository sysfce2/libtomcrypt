...
print_hex("q", tmp, len);
...
#else
   {
      ...
      for (label_hash = 0; label_hash < TAB_SIZE; ++label_hash) {
         ...
         for (rsa_msgsize = 1; rsa_msgsize <= max_msgsize; rsa_msgsize++) {
         ...
         for (rsa_msgsize = 1; rsa_msgsize <= max_msgsize; rsa_msgsize++) {
            len  = sizeof(out);
            len2 = rsa_msgsize;
            DO(rsa_encrypt_key_ex(in, rsa_msgsize, out, &len, lparam, sizeof(lparam), &yarrow_prng, prng_idx, mgf_hash, label_hash, LTC_PKCS_1_OAEP, &key));
            /* change a byte */
            out[8] ^= 1;
